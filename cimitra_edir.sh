...
if [ $DEBUG -eq 1 ]
then
	echo "In Function: $FUNCNAME" 1>> $TEMP_FILE
fi

if [ $DEBUG -eq 1 ]
then
	echo "In Function: $FUNCNAME | GroupNameIn = $GroupNameIn" 1>> $TEMP_FILE
fi

if [ $DEBUG -eq 1 ]
then
	echo "In Function: $FUNCNAME | GroupContextIn = $GroupContextIn" 1>> $TEMP_FILE
fi

if [ $DEBUG -eq 1 ]
then
	echo "In Function: $FUNCNAME | UserIdIn = $UserIdIn" 1>> $TEMP_FILE
fi

if [ $DEBUG -eq 1 ]
then
	echo "In Function: $FUNCNAME | UserContextIn = $UserContextIn" 1>> $TEMP_FILE
fi
...
then
	if [ $DEBUG -eq 1 ]
	then
		echo "In Function: $FUNCNAME | Group Does Not Exist" 1>> $TEMP_FILE
	fi
...
cat ${OUT_FILE_ONE} 1>> $TEMP_FILE
echo "grep -ic 'cn=${UserIdIn},${UserContextIn}' ${OUT_FILE_ONE}" 1>> $TEMP_FILE
...
then
	if [ $DEBUG -eq 1 ]
	then
		echo "In Function: $FUNCNAME | User Does Not Exist In Group" 1>> $TEMP_FILE
	fi
	rm ${OUT_FILE_ONE} 1> /dev/null 2> /dev/null
	echo "1"
	return
	...
	if [ $DEBUG -eq 1 ]
	then
		echo "In Function: $FUNCNAME | User Does Exist In Group" 1>> $TEMP_FILE
	fi
...
if [ $DEBUG -eq 1 ]
then
	echo "GroupNameIn = $GroupNameIn" 1>> ${TEMP_FILE}
fi

if [ $DEBUG -eq 1 ]
then
	echo "GroupContextIn = $GroupContextIn" 1>> ${TEMP_FILE}
fi
...
then
	...
	then
		...
		OutputMessage "LAST LOGIN TIME: Never Logged In"
	fi
...
GROUPS_PARAMETER_JSON='{"paramtype":5,"required":false,"multipleParamDelim":";","maxVisibleLines":4,"private":false,"allowUnmasking":false,"encapsulateChar":"\"","allowed":[_LIST_OF_GROUPS_],"param":"-GroupDNList","value":"","label":"GROUPS","regex":"/^[a-zA-Z0-9\\-\\+\\=\\_ ]+$/"}'
...
then
       ...
       {
	## Old Method
	ldapsearch -v -x -w ${EDIR_AUTH_STRING}  -D "${EDIR_USER}" -h ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_ADDRESS_ONE} -p ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_PORT_ONE} -b "${theContext}" '(&(objectClass=user)(cn=*))' 1> ${OUT_FILE_ONE} 
	##
	## New Method
	# ldapsearch -v -x -w ${EDIR_AUTH_STRING}  -D "${EDIR_USER}" -h ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_ADDRESS_ONE} -p ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_PORT_ONE} -b "${theContext}" '(&(objectClass=user)(cn=*))' 1> ${OUT_FILE_ONE} 
	##
	...
	{
	ldapsearch -v -x -w ${EDIR_AUTH_STRING}  -D "${EDIR_USER}" -h ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_ADDRESS_ONE} -p ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_PORT_ONE} '(&(objectClass=user)(cn=*))' 1> ${OUT_FILE_ONE}
	}  2> /dev/null
fi

## Old Method
cat ${OUT_FILE_ONE} | grep -i "uid:" | sed 's/uid: //' 1> ${OUT_FILE_TWO}
##

# cat ${OUT_FILE_ONE} | tr [A-Z] [a-z] | grep "dn: cn=" | cut -d '=' -f 2 | sed 's/,ou//' 1> ${OUT_FILE_TWO}
...
if [ $USER_EXISTS -eq 0 ]
then
	OutputMessage "User Already Exists: cn=${UserId},${Context}"
	OutputMessage ""
...
{
ldapdelete -v -x -h ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_ADDRESS_ONE} -p ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_PORT_ONE} -c -w ${EDIR_AUTH_STRING} -D "${EDIR_USER}" cn=${UserId},${Context}
} 1> /dev/null 2> /dev/null
...
then
	OutputMessage "---------------------------------------"
	OutputMessage "User: cn=${UserId},${Context} | Removed"
else
	OutputMessage "---------------------------------------"
	OutputMessage "User: cn=${UserId},${Context} | NOT Removed"
	OutputMessage ""
fi
