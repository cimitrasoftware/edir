...
if [ $EXCLUDE_NAME_LINE -eq 1 ]
then
	EXCLUDE_NAME_LINE=0
	continue
fi

declare -i DN_LINE=`echo "${USER_LINE}" | grep -c "dn:"`
...
LAST_USERID=`echo "$USER_LINE" | awk -F "cn=" '{printf $2}' | awk -F "," '{printf $1}'`
fi
...
USER_ACTION="0"
	...
	then
		...
		{
		ldapsearch -v -x -w ${EDIR_AUTH_STRING}  -D "${EDIR_USER}" -h ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_ADDRESS_ONE} -p ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_PORT_ONE} '(&(objectClass=user)(cn='${UserId}'))' -b "${Context}" 1> ${OUT_FILE_THREE}
		} 2> /dev/null

		TheFullName=`grep "fullName: " ${OUT_FILE_THREE} | head -1 | awk -F ":" '{printf $2}' | xargs` 
		# DN=`grep "dn: " ${OUT_FILE_THREE} | head -1 | awk -F ":" '{printf $2}' | xargs` 
		rm ${OUT_FILE_THREE} 1> /dev/null 2> /dev/null
			...
			then
   				echo "${TheFullName}" 1>> ${OUT_FILE_TWO}
				echo "${UserId}" 1>> ${OUT_FILE_TWO} 
				echo " " 1>> ${OUT_FILE_TWO} 
			else
				echo "${UserId}" 1>> ${OUT_FILE_TWO} 
			fi 
...
declare -i OUT_FILE_SIZE=`cat ${OUT_FILE_TWO} | wc -l`
...
declare -i GROUP_MEMBERSHIP_EXISTS=`grep -c "member:" ${OUT_FILE_TWO}`
	...
	GroupName=`echo "${GroupName}" | tr [a-z] [A-Z] | xargs`
...
then
	...
	echo "member: cn=${UserId},${Context}" 1>> ${TEMP_FILE_ONE}
			...
			if [ $SIMPLE_DISPLAY -eq 1 ]
			then
				OutputMessage "User: ${UserId} | Removed From Group: ${GroupName}"
			else
				OutputMessage ""
...
OutputMessage ""
OutputMessage "User: cn=${UserId},${Context}"
OutputMessage "Removed From The Group: cn=${GroupName},${GroupContext}"
fi
{
ldapmodify -v -x -w ${EDIR_AUTH_STRING}  -D "${EDIR_USER}" -h ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_ADDRESS} -p ${EDIR_LDAP_SERVICE_SIMPLE_AUTHENTICATION_PORT} -f ${TEMP_FILE_ONE}
} 1> /dev/null 2> /dev/null
