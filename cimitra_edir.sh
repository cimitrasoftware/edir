...
then
	...
	then
		echo "${USER_LINE}    ${LAST_USERID}" 1>> ${OUT_FILE_TWO} 
	else
		
		echo "${LAST_USERID}" 1>> ${OUT_FILE_TWO} 
	fi
	}
continue
else
...
then
    OutputMessage "USERS IN ORGANIZATION REPORT"
    OutputMessage "----------------------------"
    column -t ${OUT_FILE_TWO} 2> /dev/null
    OutputMessage "----------------------------"
else
    OutputMessage "USERS IN ORGANIZATION REPORT"
    OutputMessage "------------------------------"
...
declare -i GROUP_LENGTH=`echo "${GroupName},${GroupContext}" | wc -m`
	...
	then
		OutputMessage "No Group Specified"
	else
...
do
	...
	then
		...
		if [ $SIMPLE_DISPLAY -eq 1 ]
		then
			OutputMessage "User Already Exists In Group: ${GroupName}"
		else
			OutputMessage ""
...
do
	...
	then
		...
		then
			...
			OutputMessage "Error: User Does Not Exist: cn=${UserId},${Context}"
			continue
		fi
	fi
...
if [ $BULK_USER_ACTION -gt 0 ]
then
	UserIdList=`echo "$TheUserIdList" | sed -e 's/\s\+/,/g' | sed 's/,\{2,\}/,/g'`
