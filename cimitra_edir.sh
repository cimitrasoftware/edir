...
declare -i SUPPRESS_STANDARD_ERROR=0
...
while [ $# -gt 0 ]; do
  case "$1" in
	...
	DefaultManagerSearchContext="$2"
	  ;;
    -WildCardSearch)
     # If -WildCardSearch is passed along with -Action "SearchForUser" partial searches like -OfficePhone can be something like this "801" and all numbers with 801 at the beginning will meet the search criteria.
     # # -Example: -Action "SearchForUser" -WildCard Search -Context "ou=users,o=cimitra" -OfficePhone "801"
	WildCardSearch="$2"
	WILD_CARD_SEARCH=1
	  ;;
    -UserId)
